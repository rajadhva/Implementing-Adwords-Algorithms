...
def mssv(query_advertiser,advertiser_budget,queries):
    revenue=[]
    main_revenue=0
    total_budget=advertiser_budget.copy()
    for i in range(100):
        print i
        if(i!=0):
            random.seed(0)
        ...
        for query in queries:
            ...
            for advertiser in advertisers:
                if current_budget[advertiser] - query_advertiser[query][advertiser] > 0:
                    #print total_budget[advertiser]-current_budget[advertiser]
                    ratio=(query_advertiser[query][advertiser]*(1-(math.exp((total_budget[advertiser]-current_budget[advertiser])/total_budget[advertiser]-1))))
                    #print ratio
                    if(ratio>max):
                        max=ratio
                        temp_advertiser=advertiser
...
def mssvoutput():
   main_revenue,average_revenue=mssv(query_advertiser,advertiser_budget,queries)
   #print optimal_revenue
   print "The revenue in mssv is" + " : " + str(average_revenue)
   print "The competitive ratio in mssv is" + " : " + str(average_revenue/optimal_revenue)
...
#calling  the functons to print the output
